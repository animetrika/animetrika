...
// ... (compressImage remains the same)
const compressImage = async (file: File): Promise<File> => {
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            canvas.toBlob((blob) => { if (blob) resolve(new File([blob], file.name, { type: 'image/jpeg' })); else reject(new Error("Compression failed")); }, 'image/jpeg', 0.8); 
        };
...
export const updateGroupInfo = async (chatId: string, name?: string): Promise<Chat> => {
    const res = await api.put(`/groups/${chatId}`, { name });
    return res.data;
};
...
export const createChannel = async (name: string, description: string): Promise<Chat> => {
...
export const getRegistrationStatus = async (): Promise<boolean> => {
    ...
    return res.data.registrationEnabled;
};

export const toggleRegistration = async (enabled: boolean): Promise<void> => {
    await api.post('/system/config/registration', { enabled });
};
